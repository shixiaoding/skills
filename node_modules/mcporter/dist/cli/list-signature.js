...
export function formatFunctionSignature(name, options, outputSchema, formatOptions) {
...
export function formatOptionalSummary(hiddenOptions, options) {
...
export function formatFlagUsage(options, extras, opts) {
...
export function formatFlagLabel(option) {
    return `--${option.cliName} ${option.placeholder}`.trim();
}
export function formatCallExpressionExample(serverName, toolName, options, extra) {
...
export function formatExampleBlock(examples, options) {
...
function truncateExample(example, maxLength) {
...
function formatInlineParameter(option, colorize) {
...
function quoteShellExpression(expression) {
...
function inferReturnTypeName(schema) {
    if (!schema || typeof schema !== 'object') {
        return undefined;
    }
    return inferSchemaDisplayType(schema);
}
function inferSchemaDisplayType(descriptor) {
    ...
    if (!type && descriptor.items && typeof descriptor.items === 'object') {
        return `${inferSchemaDisplayType(descriptor.items)}[]`;
    }
    if (type === 'array' && descriptor.items && typeof descriptor.items === 'object') {
        return `${inferSchemaDisplayType(descriptor.items)}[]`;
    }
    if (!type && Array.isArray(descriptor.enum)) {
        const values = descriptor.enum.filter((entry) => typeof entry === 'string');
        if (values.length > 0) {
...
function formatTypeAnnotation(option, colorize) {
    let baseType;
    ...
    const tint = colorize ? dimText : (value) => value;
...
//# sourceMappingURL=list-signature.js.map